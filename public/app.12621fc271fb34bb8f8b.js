...
const initializeAppModules = () => {
    ...
    // Local Network Access Permission Testing Widgets
    addModule({
        name: "localNetworkAccess.queryPermission",
        initializedRequired: true,
        hasOutput: true,
        onClick: () => app_awaiter(void 0, void 0, void 0, function* () {
            try {
                if (!navigator.permissions) {
                    return JSON.stringify({
                        ...
                        supported: false,
                        message: "Your browser does not support the Permissions API"
                    }, null, 2);
                }
                ...
                return JSON.stringify({
                    ...
                    supported: true,
                    message: `Permission state: ${result.state}`
                }, null, 2);
            }
            catch (error) {
                return JSON.stringify({
                    ...
                    supported: false,
                    message: "local-network-access permission is not supported in this browser. Supported in Chrome 142+ and Edge 142+."
                }, null, 2);
            }
        }),
    });
    addModule({
        name: "localNetworkAccess.testLocalhost",
        initializedRequired: true,
        hasOutput: true,
        inputs: [
            {
        ...
        onClick: (port) => app_awaiter(void 0, void 0, void 0, function* () {
            ...
            try {
                ...
                // targetAddressSpace is a Chrome-specific option
                ...
                return JSON.stringify({
                    ...
                    timestamp: new Date().toISOString(),
                    message: "Request successful! The permission was granted and the server responded."
                }, null, 2);
            }
            catch (error) {
                const duration = Math.round(performance.now() - startTime);
                return JSON.stringify({
                    ...
                    timestamp: new Date().toISOString(),
                    troubleshooting: {
                        permissionDenied: "If permission was denied, you may need to reset it in browser settings",
                        serverNotRunning: "Make sure a server is running on localhost:" + port,
                        corsIssue: "The server must send 'Access-Control-Allow-Origin' and 'Access-Control-Allow-Private-Network: true' headers"
                    }
                }, null, 2);
            }
        }),
    ...
    addModule({
        name: "localNetworkAccess.testPrivateIP",
        initializedRequired: true,
        hasOutput: true,
        inputs: [
            {
        ...
        onClick: (ipAddress, port) => app_awaiter(void 0, void 0, void 0, function* () {
            const startTime = performance.now();
            try {
                ...
                if (!match) {
                    return JSON.stringify({
                        ...
                        message: "IP address must be in the format: xxx.xxx.xxx.xxx"
                    }, null, 2);
                }
                ...
                // Identify IP range type
                let ipRangeType = "unknown";
                if (octets[0] === 10) {
                    ipRangeType = "Class A Private (10.0.0.0/8)";
                }
                else if (octets[0] === 172 && octets[1] >= 16 && octets[1] <= 31) {
                    ipRangeType = "Class B Private (172.16.0.0/12)";
                }
                else if (octets[0] === 192 && octets[1] === 168) {
                    ipRangeType = "Class C Private (192.168.0.0/16)";
                }
                else if (octets[0] === 169 && octets[1] === 254) {
                    ipRangeType = "Link-local (169.254.0.0/16)";
                }
                else if (octets[0] === 127) {
                    ipRangeType = "Loopback (127.0.0.0/8)";
                }
                const portNum = parseInt(port, 10);
                if (isNaN(portNum) || portNum < 1 || portNum > 65535) {
                    return JSON.stringify({
                ...
                const url = `http://${ipAddress}:${portNum}`;
                // targetAddressSpace is a Chrome-specific option
                const response = yield fetch(url, {
                    mode: 'cors',
                    targetAddressSpace: 'private'
                ...
                return JSON.stringify({
                    ...
                    duration: `${duration}ms`,
                    timestamp: new Date().toISOString(),
                    message: "Request successful! The permission was granted and the device responded."
                }, null, 2);
            }
            catch (error) {
                const duration = Math.round(performance.now() - startTime);
                return JSON.stringify({
                    ...
                    troubleshooting: {
                        permissionDenied: "If permission was denied, you may need to reset it in browser settings",
                        deviceNotReachable: "Make sure the device at " + ipAddress + " is online and reachable",
                        firewallBlocking: "Check if firewall is blocking the connection",
                        corsIssue: "The device must send 'Access-Control-Allow-Origin' and 'Access-Control-Allow-Private-Network: true' headers"
                    }
                }, null, 2);
            }
        }),
    ...
    addModule({
        name: "localNetworkAccess.testCustomURL",
        initializedRequired: true,
        hasOutput: true,
        inputs: [
            {
        ...
        onClick: (url, method) => app_awaiter(void 0, void 0, void 0, function* () {
            const startTime = performance.now();
            try {
                // Validate URL format
                let urlObj;
                try {
                    ...
                    return JSON.stringify({
                        ...
                        message: "Please provide a valid URL (e.g., http://localhost:3000)"
                    }, null, 2);
                }
                ...
                const response = yield fetch(url, {
                    method: upperMethod,
                    mode: 'cors',
                    targetAddressSpace: isLocal ? 'private' : undefined
                });
                const duration = Math.round(performance.now() - startTime);
                const headers = {};
                response.headers.forEach((value, key) => {
                ...
                return JSON.stringify({
                    ...
                    timestamp: new Date().toISOString(),
                    troubleshooting: {
                        permissionDenied: "If accessing local/private address, permission may have been denied",
                        serverNotReachable: "Make sure the server is running and accessible",
                        corsIssue: "For local/private addresses, server must send 'Access-Control-Allow-Origin' and 'Access-Control-Allow-Private-Network: true' headers",
...
/******/ })()
;
//# sourceMappingURL=app.12621fc271fb34bb8f8b.js.map