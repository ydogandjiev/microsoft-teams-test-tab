...
export const initializeAppModules = () => {
  try {
    ...
    addModule({
      ...
      action: function (navBarMenuItems: [microsoftTeams.menus.MenuItem], output) {
        output("total States: " + totalStates);
        microsoftTeams.menus.setNavBarMenu(navBarMenuItems, function () {
          if (totalStates > 0) {
            totalStates--;
            output("microsoftTeams.menus.setNavBarMenu. total remaining state: " + totalStates);
            return true;
          }
          return false;
        });
